import {
  Bridge,
  log,
} from '../src/agents/lich0821-WeChatFerry-3091027.js'
import os from 'os'
...
const main = async () => {
  ...
  bridge.on('message', (message: MessageRaw) => {
    ...
    if (text.indexOf('46.联系人列表') > -1) {
      const connects = bridge.client.getContacts()
      console.info('connects:', connects)
    }
  })
  ...
  bridge.on('login', (data) => {
    ...
    const userinfo = bridge.client.getUserInfo()
